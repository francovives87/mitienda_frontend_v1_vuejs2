      ...
      <div class="row d-flex align-items-center">
        ...
        >
          <span class="title_bar"
            ><font-awesome-icon icon="fa-eye" /> TIENDAS VISITADAS
          </span>
        </div>
        <div class="col-4 col-sm-4 col-lg-5"><hr /></div>
        <div class="row">
                    ...
                    <div class="row">
                      ...
                      >
                        <div class="row">
                          ...
                          <div class="info_item_avg">
                            <div class="rating_container">
                              ...
                              <span
                                ...
                                >{{
                                  (
                                    Math.round(tienda.tienda.average.rating__avg * 100) /
                                    100
                                  ).toFixed(1)
                                }}</span
                              >
                          ...
                          <div class="col-12 col-sm-12">
                            <span class="tienda_title"> {{ tienda.tienda.title }}</span>
                            <p class="tienda_description">
                              {{ tienda.tienda.description }} <br />
                              <span class="location" v-if="tienda.tienda.geo_tienda[0]">
                                {{ tienda.tienda.geo_tienda[0].ciudad }}</span
                              >
                              <br />
                              <span class="address" v-if="tienda.tienda.geo_tienda[0]">{{
                                tienda.tienda.geo_tienda[0].direccion
                              }}</span>
                            </p>
                          </div>
                        </div>
...
export default {
  ...
  created() {
    this.GetTiendaVisitor();
  },

  methods: {
    GetTiendaVisitor() {
      axios
        .get(this.server + "/api/v1.0/store/visitor/all/", {
          params: {
            visitor: localStorage.getItem("visitor"),
          },
        })
        .then((response) => {
          console.log("GetTiendaVisitor");
