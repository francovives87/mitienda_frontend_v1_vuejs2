...
export default {
  name: "admin_service_edit_images",
  components: {
    admin_menu,
  },
  props: ["service_id"],
  ...
  methods: {
    ...
    CreateProductMoreImages() {
      ...
      data.append("service", this.service_id);
    ...
    GetCantImagesOffProduct() {
      axios
        .get(
          this.server +
            "/api/v1.0/admin/service/images/count/?service=" +
            this.service_id
        )
        .then((response) => {
          ...
          if (this.cant_images > 0) {
            this.allow_create =
              this.tienda.plan.images_x_services - this.cant_images;
            console.log("se pueden crear");
            console.log(this.allow_create);
          }
        })
        .catch((error) => {
          this.allow_create = this.tienda.plan.images_x_services;
        });
    },

    GetImagesOffProduct() {
      axios
        .get(this.server + "/api/v1.0/admin/service/images/" + this.service_id)
        .then((response) => {
    ...
    GetProduct() {
      axios
        .get(this.server + "/api/v1.0/admin/service/view/" + this.service_id)
        .then((response) => {
          console.log(response.data);
        ...
        .then((result) => {
          if (result.isConfirmed) {
            axios
              .delete(
                this.server +
                  "/api/v1.0/admin/service/delete/images/" +
                  image.id,
                {
                  headers: {
                    Authorization: localStorage.getItem("access"),
    ...
    UpdatePortadaImage() {
      ...
      axios
        .put(
          this.server + "/api/v1.0/admin/service/image/" + this.service_id,
          data,
          {
            headers: {
              Authorization: localStorage.getItem("access"),
    ...
    UpdateImageOffProduct() {
      let data = new FormData();
      data.append("service", this.product.id);
      ...
      axios
        .put(
          this.server +
            "/api/v1.0/admin/service/edit/images/" +
            this.ImageToEditID,
          data,
          {
            headers: {
